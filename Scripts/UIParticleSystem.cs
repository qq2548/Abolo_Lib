...
namespace AboloLib
{
    ...
    public class UIParticleSystem : MaskableGraphic
    {
        ...
        [SerializeField]
        private bool isRandomStartFrame;
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                if (_textureSheetAnimation.enabled)
                {
                    float frameProgress = 1.0f - _textureSheetAnimation.frameOverTime.curve.Evaluate(particle.remainingLifetime / particle.startLifetime);
                    // TODO - once Unity allows MinMaxCurve reading
                    frameProgress = Mathf.Repeat(frameProgress * _textureSheetAnimation.cycleCount, 1);
                                            //+ Mathf.FloorToInt(_textureSheetAnimation.startFrame.constant);
                    int frame = 0;
                    ...
                    ////暂时不支持sprite序列
                    //if (_textureSheetAnimation.mode == ParticleSystemAnimationMode.Sprites)
                    //{
                    //    Sprite spr = _textureSheetAnimation.GetSprite(frame% _textureSheetAnimationFrames);
                    //}

                    //起始帧随机序列帧
                    if (isRandomStartFrame)
                    {
                        frame += Mathf.Abs((int)(particle.randomSeed) % _textureSheetAnimationFrames);
                    }
                    frame %= _textureSheetAnimationFrames;
                    frame += StartFrameOverride;//_textureSheetAnimation.startFrame.constant 取到的值始终为0 没搞懂为什么，无奈增加一个成员变量
                    particleUV.x = (frame % _textureSheetAnimation.numTilesX) * _textureSheedAnimationFrameSize.x;
                    particleUV.y = 1.0f - Mathf.FloorToInt(frame / _textureSheetAnimation.numTilesX + 1) * _textureSheedAnimationFrameSize.y;
