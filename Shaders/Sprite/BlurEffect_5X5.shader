    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f input) : SV_Target
            {
                ...
                //遍历算卷积，25 * 2 次采样，性能表现有待观察
                for(int num = 0 ; num<25 ; num++)
                {
                    channel = tex2D(_MainTex, input.uv + Box25[num] * _Range * scalar * rect) ;
                    //channel2 = tex2Dproj(_GrabTexture, input.grabPos + Box25[num] * _Range * 0.5 * scalar * float4(_GrabTexture_TexelSize.xy , 1.0 ,1.0)) ;

                    blur += channel * GaussianSobel25[num];// + channel2 * GaussianSobel25[num] ;
                }
