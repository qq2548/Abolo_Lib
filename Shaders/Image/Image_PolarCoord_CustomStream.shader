Shader "2d/Image_PolarCoord_CustomStream"

{
	Properties
	{
		...
		_Color ("TintColor", Color) = (1.0,1.0,1.0,1.0)
		 ...
		 [KeywordEnum(Default ,Wave)]_Pattern("Effect" , Float) = 0.0
	...
	SubShader 
	{
		...
		Pass 
		{
			...
			#pragma multi_compile _PATTERN_DEFAULT _PATTERN_WAVE
			...
			fixed4 frag(v2f IN) : SV_Target
			{
                ...
                //st *= aspectRatio;
				float fac_thick = (1.0 - IN.uv.z) * 2.0;
                float d = length(max(abs(st) - _DistanceOffset , 0.0)) ;  
				float a = _DistanceDividsion - fac_thick;
				...
				color.a *= smoothstep(1.05 , 0.95 , length(st));
				#if _PATTERN_DEFAULT
					fixed4 sub_color = tex2D(_Subtex , mul(IN.sub_uv - 0.5 , ABL_2dRotationMatrix(0.1 , t * _Speed)));
				...
				#if _PATTERN_WAVE
					fixed4 sub_color = tex2D(_Subtex , float2(polar , channel + t * _Speed));
