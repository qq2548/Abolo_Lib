...

{
	Properties
	{
		...
		_ChannelOffset ("ChannelOffset", Float) = 0.1
	...
	SubShader 
	{
		...
		Pass 
		{
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				fixed Rchannel = (tex2D(_MainTex, float2(polar , channel + _ChannelOffset )) * IN.Color).r;
				fixed Bchannel = (tex2D(_MainTex, float2(polar , channel - _ChannelOffset)) * IN.Color).b;
				fixed4 sub_color = tex2D(_Subtex , mul(IN.sub_uv - 0.5 , ABL_2dRotationMatrix(0.1 , t)));
				
				fixed mask = polar;
				mask = saturate(mask  + sub_color.r) * (1 - smoothstep(0.9 , 1 , length(coord * 2.0)));
				color = fixed4(Rchannel , color.g , Bchannel , color.a);
